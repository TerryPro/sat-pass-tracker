...
def _fallback_tle():
    return (*_SATELLITES[24278]["fallback"], True)


def _online_tle():
    name, l1, l2 = _SATELLITES[24278]["fallback"]
    return (name, l1, l2, False)


def test_online_source(monkeypatch):
    monkeypatch.setattr(tle, "fetch_latest_tle", lambda norad_id=24278: _online_tle())
    tle._get_tle_cached(24278)
    assert tle.tle_source(24278) == "online"


def test_fallback_source(monkeypatch):
    monkeypatch.setattr(tle, "fetch_latest_tle", lambda norad_id=24278: _fallback_tle())
    tle._get_tle_cached(24278)
    assert tle.tle_source(24278) == "fallback"


def test_file_cache_source(monkeypatch):
    """重启后从持久化文件恢复来源；旧数据无 source 字段视为 cache。"""
    monkeypatch.setattr(tle, "fetch_latest_tle", lambda norad_id=24278: _fallback_tle())
    tle._get_tle_cached(24278)  # 写入文件（source=fallback）
    # 模拟重启：清空内存缓存与来源
...
def test_legacy_cache_defaults_to_cache(monkeypatch):
    ...
    store._TLES_FILE.write_text(
        json.dumps({str(24278): {"name": name, "tle1": l1, "tle2": l2, "fetched_ts": __import__("time").time()}}),
        encoding="utf-8",
    )
    tle._get_tle_cached(24278)
    assert tle.tle_source(24278) == "cache"
